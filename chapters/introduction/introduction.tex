...
Simulations of atomic systems are an integral part of modern atomic scale research, whether it be to investigate specific electronic structure properties~\citemiss, performing atomic dynamics~\citemiss, investigating macroscopic propertiesi (e.g.~\cite{macroscopic}) or simply finding stable structures~\cite{summer-school-paper-2009}.
...
These can range, for example, from rotations of specific groups with minimal impact on the properties of the material (as seen in \fref{chap:borohydrides}) and motion of water molecules in liquid water~\citemiss to adsorption~\citemiss and the breaking of strong chemical bonds~\citemiss.
...
So superb, in fact, that any current alternative mobile storage medium is neither competitive with regards to energy density nor price. \figmiss{price}
...
These storage solutions tend to have more reasonable volumetric densities (at a reasonable cost of the gravimetric density) \figmiss{energy density} but often require "unnatural" operating conditions for loading/unloading.
