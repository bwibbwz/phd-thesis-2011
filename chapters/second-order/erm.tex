...
%\tb{(The following might be moved to the main text)}Close to the \sap{1}s, only one eigenvalue is negative and the direction of the eigenvector is perpendicular to the ridge.
%Close to the \sap{2}, there are two negative eigenvalues and in a region to the left of the \sap{2} the eigenmode corresponding to the lower eigenvalue is parallel to the ridge.
%A region where neither of the two eigenvectors corresponding to negative eigenvalues are perpendicular to the ridge can be seen below the \sap{2}.
}
\label{fig:modes}
\end{center}
\end{figure}
...
By transforming the force, $\vF^\perp$, in such a way that it locally, near the ridge, corresponds to that of a MEP %(i.e. a `valley')
\begin{equation}
...
In order to perform the force transformation described in \eref{eq:force-mapping}, it is necessary to be able to find the eigenmode associated with the lowest eigenvalue (hereafter referred to as the minimum mode) of the reduced Hessian matrix.
...
For clarity, we rewrite \eref{eq:force-mapping} for each image, $i$
\begin{equation}
...
\caption{
The construction of the effective force, $\vF_i^\text{eff}$, used in the iterative optimization for finding an energy ridge, from \eref{eq:effective-force}.
$\vF_i$ (orange) is the real force, $\vF_i = -\nabla E_i$ acting on the image,
$\vF^t_i$ (red) is the transformed force, inverted in the direction of the minimum mode, from \eref{eq:minmode-force},
...
$\vS_i$ (green) is the spring force, from \eref{eq:full-spring-force},
$\vF^\text{eff}_i$ (blue) is the effective force acting on the image, from \eref{eq:effective-force}.
...
\begin{equation}
%\vF_i^\text{eff} = \vF_i^t - (\vF_i^t \cdot \uvt_i) + \vF_i^\text{S}
\vF_i^\text{eff} = \vF_i^t + \vF_i^\text{S}
\label{eq:effective-force}
\end{equation}
where the first term is the transformed force from \eref{eq:minmode-force}%,
%the second term removes any components of the transformed force parallel to the path,
and the %third 
second term is the spring force from \eref{eq:full-spring-force} that controls the distribution of the images along the path and increases numerical stability.
...
%In the NEB method, the perpendicular component of the spring force is projected out, but we have chosen to keep it to improve the stability of the iterative optimization. \tred{I'd prefer some elaboration on this point, if possible without giving away the "containment" force.}
...
%By turning on the climbing image \cite{neb-ci-2000} algorithm for the highest energy image in the path,
% after the RMS force on the images has dropped to below $??? \unit{eV/\mAA}$
%a convergence to the highest \sap{2} along the path, which usually is the most interesting one for the present study is obtained even with the full spring force. \tb{(REWRITE THE ABOVE SENTENCE)}
%The climbing image \cite{neb-ci-2000} algorithm decouples the highest energy image from the springs and inverts the force along the tangent in a manner similar to \eref{eq:minmode-force}\cite{neb-ci-2000},
This is accomplished by decoupling the highest energy image from the springs and inverting the force along the tangent in a manner similar to \eref{eq:minmode-force},
\begin{equation}
...
%It should be noted that only the top energy image of the path can be safely decoupled from the containment force in this manner.
%The rest of the images might still get placed off the ridge where especially where it has large curvature due to corner-cutting.
...
Black: The effective force, given by \eref{eq:effective-force} or \eref{eq:neb-ci}, which is used in the iterative optimization.
...
%In order to maintain as much numerical stability as possible, the full spring force is used, as described above.
...
%However, it is difficult to do a direct comparison.
The ridge calculation converges, furthermore, slower than a MEP calculation
%the underlying double ended \sap{1} method 
...
%In simple test calculations both preformed well.
%An example of these calculations, comparing two symmetrically identical Al adatom hop precesses on an Al(100) surface, calculated using the %effective medium theory potential function\cite{emt-1996}, 
%embedded atom model can be seen in \fref{fig:adatom-snapshots}.
%A more detailed study of this system is presented below including diffusion via concerted motion of surface atoms\cite{concerted-motion-1990} .
