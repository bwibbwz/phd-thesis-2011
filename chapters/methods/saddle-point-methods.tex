...
%\vF^\circlearrowright = (\vF_\text{A} - (\vF_\text{A} \cdot \vs)\vs) - (\vF_\text{B} - (\vF_\text{B} \cdot \vs)\vs)
\vF^\circlearrowright = \vF_\text{A} - \vF_\text{B} + ((\vF_\text{B} - \vF_\text{A}) \cdot \vs)\vs,
\eeq
...
Often $\vF_0$ needs to be calculated, this can be taken advantage of in order to cut down the amount of computations.
...
Further extrapolations are possible ... if multiple rotations are required (which is often not the case)...
