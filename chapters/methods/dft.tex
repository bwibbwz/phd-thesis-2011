...
First there is the kinetic energy of the electrons, $\widehat{T}_e = -2^{-1}\sum_i^{N_e}\nabla_i^2$,
...
\eeq
since these are system dependant, while the others can be considered universal.

%\beq{hamitonian-shorthand}
%\widehat{H} = \widehat{T}_e - \widehat{U}_{ee} -  \widehat{V}_\text{ext}.
%\eeq
...
The earliest mention of a DFT type method were made by Thomas~\citemiss and Fermi~\citemiss but the heart of DFT lies in the Hohnberg-Kohn theorem, which reformulates the electronic structure problem to depend on the $3$ dimensional, ground state, electron density, $\rho(\vr)$, instead of the $3N$ dimensional wavefunction, $\Psi(\vr_1, \vr_2, \ldots, \vr_N)$.
...
%E[\rho_0] = \bra \Psi[\rho_0] | \widehat{T}_e + \widehat{U}_{ee} + \widehat{V}_\text{ext} | \Psi[\rho_0] \ket,
%\eeq
...
\end{equation}
where $v(\vect{r})$ is the sum of the potential due to the kinetic and ionic parts in equation \ref{eq:hamiltonian-full}%[Is this correct and do I need to include this reference]
