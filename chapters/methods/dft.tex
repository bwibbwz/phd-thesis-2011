...
One of the most popular and common teqchniques is the Density Functional Theory (DFT) which can, from first principles (\textit{ab initio}), exactly (in theory) solve the Schr\"odinger equation (see \fref{sec:schrodinger} below).
However, in practice, approximations must be made for any but the most simple systems.
...
\unread
%With the advances in computing power in recent years, both with more powerful computers and, in particular, parallel computing, intensive calculations have become more viable.
%One such method is Density Functional Theory (DFT), which can be used to approximate a solution to the Scr\"odinger equation (section \ref{sec:schrodinger}) for many-electron systems using \textit{ab initio} (from the beginning) methods. 
