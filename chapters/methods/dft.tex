...
A non-relativistic system of N electrons is described by the time independant Schr\"odinger equation,
\beq{eq:schrodinger-basic}
 \widehat{H}\Psi = E\Psi,
\eeq
...
The hamiltonian is often separated into three terms: the kinetic term,
\beq{eq:hamiltonian-kinetic}
 \widehat{K} = \sum_i^N -\frac{\nabla^2_i}{2},
\eeq
the electron-electron interaction term,
\beq{eq:hamiltonian-electron-electron}
 \widehat{V}_\text{e-e} = \sum_i^N\sum_{j>i}^N\frac{1}{\left| \vr_i - \vr_j \right|},
\eeq
and the electron-environment interaction term,
\beq{eq:hamiltonian-environment}
 \widehat{V}_\text{env} = \sum_i^N \sum_I^{N_I} \frac{1}{\left| \vr_i - \vR_I \right|} + V_\text{ext},
\eeq
where the $N_I$ is the number of nuclei and $V_\text{ext}$ is any further external influence, e.g. an applied electric field.
Combining the above terms, yields the full hamiltonian,
\beq{eq:hamiltonian}
 \widehat{H} = \widehat{K} + \widehat{V}_\text{e-e} + \widehat{V}_\text{env}.
\eeq

Innocent it may look but solving the Schr\"odinger equation is a daunting task and in most cases significant approximations must be made.
