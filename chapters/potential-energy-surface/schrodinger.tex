...
\beq{schrodinger-basic}
 \widehat{H}\Psi = E\Psi,
\eeq
must be solved in order to calculate the total energy of the system, $E$, and the wave function, $\Psi \equiv \Psi(\vr_1, \vr_2, \ldots, \vr_{N_e}, \vR_1, \vR_2, \ldots, \vR_{N_n})$, which depends on the spatial coordinates of each electron, $\vr_i$, and each nucleus, $\vR_i$, using the total energy operator (more commonly referred to as the Hamiltonian),
\beq{hamiltonian}
\widehat{H} = \sum_i^{N}\widehat{T}_i  + \widehat{V}.
\eeq
Similar to classical systems the Hamiltonian encompasses both kinetic energy, $\widehat{T}_i = -\nabla_i^2/2$, \footnote{In atomic units} and potential energy, $\widehat{V} = V(\vR, \vr)$, represented in position space, which will be used exclusively in this thesis.
...
Efforts to solve the Schr\"odinger equation are discussed in \fref{sec:methods-qm}.
