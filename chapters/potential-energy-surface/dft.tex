...
Solving for $\phi_i$ and $\epsilon_i$ (the Kohn-Sham energies) using multiple Schr\"odinger type equations,
...
The electron-electron interaction, $\widehat{U}_\text{ee}$, enters in a changed form.
In fact it is introduced as two terms, one for interaction with the electron density, commonly referred to as the hartree potential ($v_\text{H}(\vr)$) and a second one for self-interaction, $v_\text{XC}(\vr)$,
...
Thus far the formalism is exact, however, all the difficult parts, $v_\text{XC}$, have yet to be discussed.
